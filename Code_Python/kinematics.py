...
def j_num(num_link, BB):
    """Return the link sequence from the base (excluded) to the endpoint associated with
       the link defined by number <num_link>"""
...
def f_kin_e(RR, AA, seq_link, Qe, ce):
    """Return position and orientation of the endpoint defined by sequence <seq_link>
...
def f_kin_j(RR, AA, q, seq_link, j_type, cc, Ez):
    """Return position and orientation of all joints in sequence <seq_link> (the link
       number of this endpoint is the last value in <seq_link>)"""
...
def calc_jte(RR, AA, q, seq_link, j_type, cc, ce, Qe, Ez):
    """Return the translational Jacobian (3 x n_links) of the endpoint defined by sequence 
       <seq_link> (the link number of this endpoint is the last value in <seq_link>)"""
...
def calc_jre(AA, seq_link, j_type, Ez):
    """Return the rotational Jacobian (3 x n_links) of the endpoint defined by sequence 
       <seq_link> (the link number of this endpoint is the last value in <seq_link>)"""
...
def calc_je(RR, AA, q, seq_link, j_type, cc, ce, Qe, Ez):
    """Calculation of the Jacobian (6 x num_j) of the endpoint defined by sequence
       <seq_link>"""
...
def calc_aa(Q0, q, BB, j_type, Qi):
    """Calculate the rotation matrices of all bodies (link centroid frame is set equal to
       the corresponding joint frame)"""
...
def calc_pos(R0, AA, q, BB, j_type, cc, Ez):
    """Calculate the position vector of all body centroids"""
...
def calc_vel(AA, v0, w0, q, qd, BB, j_type, cc, Ez):
    """Calculate the linear velocity vector of all body centroids and the angular velocity
       vector of all bodies"""
    ...
    for i in range(1,num_b):
        ...
        # Prismatic joint
            ...
            ww[:,i] = ww[:,k]
            vv[:,i] = vv[:,k] + cross( ww[:,k], (A_I_k @ cc[:,k,i]) ) - \
                cross( ww[:,i], (A_I_i @ cc[:,i,i]) ) + A_I_i @ (Ez*qd[idxi]) + \
...
def calc_acc(AA, ww, vd0, wd0, q, qd, qdd, BB, j_type, cc, Ez):
    """Calculate the linear acceleration vector of all body centroids and the angular
       acceleration vector of all bodies"""
        ...
        if (j_type[idxi] == 'R'):
            ...
            vd[:,i] = vd[:,k] + cross(  wd[:,k], cc_I_k ) + \
                cross( ww[:,k], cross( ww[:,k], cc_I_k ) ) - \
                    cross( ww[:,i], cross( ww[:,i], cc_I_i ) ) - cross( wd[:,i], cc_I_i )
                            
        # Prismatic joint
        elif (j_type[idxi] == 'P'):
            wd[:,i] = wd[:,k]
            vd[:,i] = vd[:,k] + cross( wd[:,k], cc_I_k ) + \
                cross( ww[:,k], cross( ww[:,k], cc_I_k ) ) + cross( wd[:,i], q_I_i ) + \
...
def calc_jt(RR, AA, BB, j_type, cc, Ez):
    """Calculate the translational jacobians wrt the link centroids (base not included)"""
...
def calc_jr(AA, BB, j_type, Ez):
    """Calculate the rotational jacobians wrt the link centroids (base not included)"""
...
def calc_hh(RR, AA, mass, inertia, BB, j_type, cc, Ez):
    """Calculate the inertia matrices HH (6 + num_j) x (6 + num_j)"""
